...
if(!function_exists('topfit_mikado_get_title')) {
	/**
	 * Loads title area HTML
	 */
	function topfit_mikado_get_title() {
		$id = topfit_mikado_get_page_id();

		extract(topfit_mikado_title_area_height());
		extract(topfit_mikado_title_area_background());
		...
		if(($meta_temp = get_post_meta($id, "mkd_show_title_area_meta", true)) !== "") {
			...
			$show_title_area = topfit_mikado_options()->getOptionValue('show_title_area') == 'yes' ? true : false;
		}
		...
		if(($meta_temp = get_post_meta($id, "mkd_title_area_type_meta", true)) !== "") {
			...
			$type = topfit_mikado_options()->getOptionValue('title_area_type');
		}
		...
		if(($meta_temp = get_post_meta($id, "mkd_title_area_enable_breadcrumbs_meta", true)) !== "") {
			...
			$enable_breadcrumbs = topfit_mikado_options()->getOptionValue('title_area_enable_breadcrumbs') == 'yes' ? true : false;
		}
        ...
        $type = topfit_mikado_intersect_title_options_with_events(topfit_mikado_options()->getOptionValue('title_events_area_type'), $type);
		...
		$parameters = apply_filters('topfit_mikado_title_area_height_params', $parameters);

		topfit_mikado_get_module_template_part('templates/title', 'title', '', $parameters);
	}
}

if(!function_exists('topfit_mikado_get_title_text')) {
	/**
	 * Function that returns current page title text. Defines topfit_mikado_title_text filter
	 ...
	 */
	function topfit_mikado_get_title_text() {
		...
		if(is_tag()) {
			...
			$title = single_term_title("", false).esc_html__(' Tag', 'topfit');
		...
		elseif(is_author()) {
			...
			$title = esc_html__('Author:', 'topfit')." ".get_the_author();
		...
		elseif(is_404()) {
			...
			if(topfit_mikado_options()->getOptionValue('404_title') != "") {
				//get it from options
				$title = topfit_mikado_options()->getOptionValue('404_title');
			} else {
				//get default 404 page title
				$title = esc_html__('404 - Page not found', 'topfit');
			}
		}//is single post page
        elseif (is_singular('post')) {
            if (topfit_mikado_options()->getOptionValue('blog_single_title_in_title_area') == "yes") {
                $title = get_the_title($id);
            } else {
                $title = esc_html__('Blog', 'topfit');
            }
        }//is WooCommerce installed and is shop or single product page?
		elseif(topfit_mikado_is_woocommerce_installed() && (is_shop() || is_singular('product'))) {
		...
		elseif(topfit_mikado_is_woocommerce_installed() && (is_product_category() || is_product_tag())) {
			global $wp_query;
		...
		elseif(is_archive()) {
			$title = esc_html__('Archive', 'topfit');
		} //current page is regular page
		else {
			$title = get_the_title($id);
		}

		$title = apply_filters('topfit_mikado_title_text', $title);

		return $title;
	}
}

if(!function_exists('topfit_mikado_title_text')) {
	/**
	 * Function that echoes title text.
	 *
	 * @see topfit_mikado_get_title_text()
	 */
	function topfit_mikado_title_text() {
		echo topfit_mikado_get_title_text();
	}
}

if(!function_exists('topfit_mikado_subtitle_text')) {
	/**
	 * Function that echoes subtitle text.
	 *
	 */
	function topfit_mikado_subtitle_text() {
		$id       = topfit_mikado_get_page_id();
		$subtitle = '';
        ...
        echo topfit_mikado_get_module_part($subtitle);
	}
}

if(!function_exists('topfit_mikado_custom_breadcrumbs')) {
	/**
	 ...
	 * @see is_front_page()
	 * @see is_category()
	 * @see topfit_mikado_is_product_category()
	 * @see get_search_query()
	 */
	function topfit_mikado_custom_breadcrumbs() {
		global $post, $wp_query;

		$output      = "";
		$homeLink    = esc_url(home_url('/'));
		$pageid      = topfit_mikado_get_page_id();
		$bread_style = "";
		...
		$delimiter   = '<span class="mkd-delimiter" '.topfit_mikado_get_inline_style($bread_style).'>&nbsp;>&nbsp;</span>'; // delimiter between crumbs
		$home        = get_bloginfo('name'); // text for the 'Home' link
		$showCurrent = 1; // 1 - show current post/page title in breadcrumbs, 0 - don't show
		$before      = '<span class="mkd-current" '.topfit_mikado_get_inline_style($bread_style).'>'; // tag before the current crumb
		$after       = '</span>'; // tag after the current crumb

		if(is_home() && !is_front_page()) {
			$output = '<div class="mkd-breadcrumbs"><div class="mkd-breadcrumbs-inner"><a '.topfit_mikado_get_inline_style($bread_style).' href="'.$homeLink.'">'.$home.'</a>'.$delimiter.' <a '.topfit_mikado_get_inline_style($bread_style).' href="'.$homeLink.'">'.get_the_title($pageid).'</a></div></div>';

		} elseif(is_home()) {
			$output = '<div class="mkd-breadcrumbs"><div class="mkd-breadcrumbs-inner">'.$before.$home.$after.'</div></div>';
		} elseif(is_front_page()) {
			if($showOnHome == 1) {
				$output = '<div class="mkd-breadcrumbs"><div class="mkd-breadcrumbs-inner"><a '.topfit_mikado_get_inline_style($bread_style).' href="'.$homeLink.'">'.$home.'</a></div></div>';
			}
		} else {

			$output .= '<div class="mkd-breadcrumbs"><div class="mkd-breadcrumbs-inner"><a '.topfit_mikado_get_inline_style($bread_style).' href="'.$homeLink.'">'.$home.'</a>'.$delimiter;

			if(is_category() || topfit_mikado_is_product_category()) {
				...
				$output .= $before.esc_html__('Search results for ', 'topfit').'"'.get_search_query().'"'.$after;

			} elseif(is_day()) {
				$output .= '<a '.topfit_mikado_get_inline_style($bread_style).' href="'.get_year_link(get_the_time('Y')).'">'.get_the_time('Y').'</a>'.$delimiter;
				$output .= '<a '.topfit_mikado_get_inline_style($bread_style).' href="'.get_month_link(get_the_time('Y'), get_the_time('m')).'">'.get_the_time('F').'</a>'.$delimiter;
				...
				$output .= '<a '.topfit_mikado_get_inline_style($bread_style).' href="'.get_permalink($parent).'">'.$parent->post_title.'</a>';
				if($showCurrent == 1) {
				...
				while($parent_id) {
					...
					$breadcrumbs[] = '<a '.topfit_mikado_get_inline_style($bread_style).' href="'.get_permalink($page->ID).'">'.get_the_title($page->ID).'</a>';
				...
				$output .= $before.esc_html__('Posts tagged ', 'topfit').'"'.single_tag_title('', false).'"'.$after;

			} elseif(is_author()) {
				global $authordata;
				$output .= $before.esc_html__('Articles posted by ', 'topfit').$authordata->display_name.$after;

			} elseif(is_404()) {
				$output .= $before.esc_html__('Error 404', 'topfit').$after;
			...
			if(get_query_var('paged')) {
				...
				$output .= $before." (".esc_html__('Page', 'topfit').' '.get_query_var('paged').")".$after;
...
if(!function_exists('topfit_mikado_get_title_area_height')) {

	/**
	 * Function that returns title height
	 **/
	function topfit_mikado_get_title_area_height() {

		$id  = topfit_mikado_get_page_id();

		$title_height = 80; // default title height without header height


        if(topfit_mikado_get_meta_field_intersect('title_area_type',$id) == 'standard'){
            $title_height = 327;
        }

		if(topfit_mikado_get_meta_field_intersect('title_area_height',$id) != '') {
			$title_height = topfit_mikado_get_meta_field_intersect('title_area_height',$id);
		}

		return apply_filters('topfit_mikado_title_area_height', $title_height);
	}
}

if(!function_exists('topfit_mikado_get_title_content_padding')) {
	/**
	 * Function that returns title content pading
	 **/

	function topfit_mikado_get_title_content_padding() {
		return apply_filters('topfit_mikado_title_content_padding', 0);
	}
}


if(!function_exists('topfit_mikado_title_area_height')) {
	/**
	 * Function that returns title height and padding to be applied in template
	 **/

	function topfit_mikado_title_area_height() {
		$id                            = topfit_mikado_get_page_id();
		$title_height_and_padding      = array();
		$title_height                  = topfit_mikado_get_title_area_height();
		$header_height_padding         = topfit_mikado_get_title_content_padding();
		$title_vertical_alignment      = 'header_bottom';
		$title_holder_height           = '';
		$title_subtitle_holder_padding = '';

        //check for tribe events options
        $title_height = topfit_mikado_intersect_title_options_with_events(topfit_mikado_options()->getOptionValue('title_events_area_height'), $title_height);
		...
		if(($meta_temp = get_post_meta($id, "mkd_title_area_background_image_responsive_meta", true)) != "") {
			...
			$is_img_responsive = topfit_mikado_options()->getOptionValue('title_area_background_image_responsive');
		}

		if(($meta_temp = get_post_meta($id, "mkd_title_area_vertial_alignment_meta", true)) !== '') {
			$title_vertical_alignment = $meta_temp;
		} else {
			$title_vertical_alignment = topfit_mikado_options()->getOptionValue('title_area_vertial_alignment');;
		}
...
if(!function_exists('topfit_mikado_title_area_background')) {
	/**
	 * Function that returns title background style be applied in template
	 **/

	function topfit_mikado_title_area_background() {
		$id                           = topfit_mikado_get_page_id();
		$show_title_img               = true;
		...
		if(($meta_temp = get_post_meta($id, "mkd_title_area_background_color_meta", true)) != "") {
			...
			$background_color = topfit_mikado_options()->getOptionValue('title_area_background_color');
		}

        //check for tribe events options
        $background_color = topfit_mikado_intersect_title_options_with_events(topfit_mikado_options()->getOptionValue('title_events_area_background_color'), $background_color);
		...
		if(($meta_temp = get_post_meta($id, "mkd_title_area_background_image_meta", true)) != "") {
			...
			$background_image = topfit_mikado_options()->getOptionValue('title_area_background_image');
		}

        //check for tribe events options
        $background_image = topfit_mikado_intersect_title_options_with_events(topfit_mikado_options()->getOptionValue('title_events_area_background_image'), $background_image);

		//check for background image width
		$background_image_width = "";
		if($background_image !== '') {
			$background_image_width_dimensions_array = topfit_mikado_get_image_dimensions($background_image);
...
function topfit_mikado_is_event_archive_page() {
...
function topfit_mikado_intersect_title_options_with_events($option, $default_value) {
    if(topfit_mikado_the_events_calendar_installed()
        && get_post_type() == 'tribe_events'
        && is_archive()
        && $option !== ''
