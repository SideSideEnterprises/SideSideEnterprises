...
if(!function_exists('topfit_mikado_single_portfolio')) {
	/**
	 * Loads holder template for portfolio single
	 */
	function topfit_mikado_single_portfolio() {
		$portfolio_template = topfit_mikado_get_portfolio_single_type();
		...
		if($portfolio_template == 'gallery') {
			$params['holder_class'][] = 'mkd-portfolio-gallery-'.topfit_mikado_options()->getOptionValue('portfolio_single_numb_columns');
		}

		topfit_mikado_get_module_template_part('templates/single/holder', 'portfolio', '', $params);
	}
}

if(!function_exists('topfit_mikado_get_portfolio_single_type')) {
	/**
	 * Returns template of current portfolio item
	 *
	 * @return bool|mixed|void
	 */
	function topfit_mikado_get_portfolio_single_type() {
		return topfit_mikado_get_meta_field_intersect('portfolio_single_template');
	}
}

if(!function_exists('topfit_mikado_get_portfolio_single_media')) {
	/**
	 * Returns a media array of current portfolio item
	 *
	 * @return array
	 */
	function topfit_mikado_get_portfolio_single_media() {
		...
		if(is_array($videos) && count($videos)) {
			usort($videos, 'topfit_mikado_compare_portfolio_videos');
			foreach($videos as $video) {
				...
				if(!empty($video['portfoliovideotype'])) {
					...
					$media['video_url']   = topfit_mikado_portfolio_get_video_url($video);
...
if(!function_exists('topfit_mikado_portfolio_get_video_url')) {
	/**
	 ...
	 * @return array|string
	 */
	function topfit_mikado_portfolio_get_video_url($video) {
...
if(!function_exists('topfit_mikado_portfolio_get_media_html')) {
	/**
	 * Loads template for current portfolio item media
	 *
	 * @param $media
	 */
	function topfit_mikado_portfolio_get_media_html($media) {
		global $wp_filesystem;
		$params = array();

		//For adding image overlay in gallery template type
		$params['gallery'] = (topfit_mikado_get_portfolio_single_type() == 'gallery') ? true : false;

		if($media['type'] == 'image') {
			$params['lightbox'] = topfit_mikado_options()->getOptionValue('portfolio_single_lightbox_images') == 'yes';
		...
		if(in_array($media['type'], array('youtube', 'vimeo'))) {
			$params['lightbox'] = topfit_mikado_options()->getOptionValue('portfolio_single_lightbox_videos') == 'yes';
		...
		topfit_mikado_get_module_template_part('templates/single/media/'.$media['type'], 'portfolio', '', $params);
	}
}

if(!function_exists('topfit_mikado_compare_portfolio_videos')) {
	/**
	 ...
	 */
	function topfit_mikado_compare_portfolio_videos($a, $b) {
...
if(!function_exists('topfit_mikado_compare_portfolio_options')) {
	/**
	 ...
	 * @return int result of comparison
	 */
	function topfit_mikado_compare_portfolio_options($a, $b) {
...
if(!function_exists('topfit_mikado_portfolio_get_info_part')) {
	/**
	 * Loads portfolio info item based on passed param
	 *
	 * @param $part
	 */
	function topfit_mikado_portfolio_get_info_part($part) {
		$portfolio_template = topfit_mikado_get_portfolio_single_type();

		topfit_mikado_get_module_template_part('templates/single/parts/'.$part, 'portfolio', $portfolio_template);
	}
}

if(!function_exists('topfit_mikado_portfolio_get_single_navigation')) {
	/**
	 *
	 */
	function topfit_mikado_portfolio_get_single_navigation() {
		$params = array();

		$in_same_term = topfit_mikado_options()->getOptionValue('portfolio_single_nav_same_category') == 'yes';
		...
		topfit_mikado_get_module_template_part('templates/single/parts/navigation', 'portfolio', '', $params);
	}
}