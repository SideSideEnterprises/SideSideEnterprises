...
/*
  Class: TopFitMikadoIconCollections
...
class TopFitMikadoIconCollections {
	...
	private function initIconCollections() {
		$this->addIconCollection('font_awesome', new TopFitMikadoIconsFontAwesome("Font Awesome", "fa_icon"));
		$this->addIconCollection('font_elegant', new TopFitMikadoIconsFontElegant("Font Elegant", "fe_icon"));
		$this->addIconCollection('ion_icons', new TopFitMikadoIonIcons("Ion Icons", "ion_icon"));
		$this->addIconCollection('linea_icons', new TopFitMikadoLineaIcons('Linea Icons', 'linea_icon'));
		$this->addIconCollection('linear_icons', new TopFitMikadoLinearIcons('Linear Icons', 'linear_icon'));
		$this->addIconCollection('simple_line_icons', new TopFitMikadoSimpleLineIcons('Simple Line Icons',
			'simple_line_icons'));
		$this->addIconCollection('dripicons', new TopFitMikadoDripicons('Dripicons', 'dripicon'));
	}
	...
	public function getSocialVCParamsArray($iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false, $exclude = '') {
		...
		$iconPackParams = array(
			'type'        => 'dropdown',
			'heading'     => esc_html__('Icon pack', 'topfit'),
		...
		if (is_array($iconCollections) && count($iconCollections)) {
			foreach ($iconCollections as $key => $collection) {
				$iconSetParams[] = array(
					'type'        => 'dropdown',
					'class'       => '',
					'heading'     => esc_html__('Icon', 'topfit'),
					'param_name'  => $iconCollectionPrefix . $collection->param,
	...
	public function getWidgetIconParams() {
		...
		$iconPackParams[] = array(
			'type'    => 'dropdown',
			'title'   => esc_html__('Icon Pack', 'topfit'),
			'name'    => 'icon_pack',
			'options' => array(
	...
	public function getIconCollectionIcons(iTopFitMikadoIconCollection $collection) {
	...
	public function enqueueStyles() {
		if (is_array($this->iconCollections) && count($this->iconCollections)) {
			foreach ($this->iconCollections as $collection_key => $collection_obj) {
                wp_enqueue_style('mkd-' . str_replace('_', '-', $collection_key), $collection_obj->styleUrl);
			}
		}
	}
	...
	public function getSearchIcon($iconPack, $return) {
		...
		if ($this->hasIconCollection($iconPack)) {
			...
			if ($return) {
                ...
                echo topfit_mikado_get_module_part($iconsObject->getSearchIcon());
			}
	...
	public function getSearchClose($iconPack, $return) {
		...
		if ($this->hasIconCollection($iconPack)) {
			...
			if ($return) {
                ...
                echo topfit_mikado_get_module_part($iconsObject->getSearchClose());
			}
	...
	public function getSearchIconValue($iconPack) {
		...
		if ($this->hasIconCollection($iconPack)) {
			...
			$iconsObject = $this->getIconCollection($iconPack);
            echo topfit_mikado_get_module_part($iconsObject->getSearchIconValue());
	...
	public function getMenuSideIcon($iconPack) {
		...
		if ($this->hasIconCollection($iconPack)) {
			...
			$iconsObject = $this->getIconCollection($iconPack);
            echo topfit_mikado_get_module_part($iconsObject->getMenuSideIcon());
		}
	...
	public function getBackToTopIcon($iconPack) {
		...
		if ($this->hasIconCollection($iconPack)) {
			...
			$iconsObject = $this->getIconCollection($iconPack);
            echo topfit_mikado_get_module_part($iconsObject->getBackToTopIcon());
	...
	public function getMobileMenuIcon($iconPack, $return = false) {
		...
		if ($this->hasIconCollection($iconPack)) {
			...
			if ($return) {
                ...
                echo topfit_mikado_get_module_part($iconsObject->getMobileMenuIcon());
			}
		}
	...
	public function getQuoteIcon($iconPack, $return = false) {
		...
		if ($this->hasIconCollection($iconPack)) {
			...
			if ($return == true) {
                ...
                echo topfit_mikado_get_module_part($iconsObject->getQuoteIcon());
			}
...
$topfit_IconCollections = TopFitMikadoIconCollections::get_instance();