...
(function ($) {
    ...
    common.mkdPreloaderCounter = mkdPreloaderCounter;
    ...
    function mkdSmoothTransition() {
        ...
        var removeLoader = function() {
            ...
            else if(!($(".filling-circle-spinner").length)) {
        ...
        if (loader.length) {
            ...
            //continue loading
            $(window).load(function(){
                mkd.body.addClass('mkd-content-loaded');
                if(mkd.body.hasClass('mkd-content-preloaded')){
                    mkdLoadingFinished();
                }
            });

            var mkdLoadingContinue = function() {
                if(mkd.body.hasClass('mkd-content-loaded')){
                    mkdLoadingFinished();
                }else{
                    mkd.body.addClass('mkd-content-preloaded');
                }
            }
    ...
    function mkdPreloaderCounter(){
        if($(".filling-circle-spinner").length){

            // Initial slow spinner
            var spinner = $(".filling-circle-spinner");
            spinner.addClass("mkd-loading-started");
            spinner.find(".filling-circle-number").countTo({
                from: 0,
                to: 100,
                speed: 4000,
                refreshInterval: 50
            ...
            $(window).load(function(){
                var spinner = $(".filling-circle-spinner"),
                    leftPie = $(".filling-circle-left .filling-circle-pie"),
                    rightPie = $(".filling-circle-right .filling-circle-pie");

                // Initial slow spinner is hidden, and faster spinner is shown
                // Preloader is speeded up after adding this class
                setTimeout(function(){
                    var tempSpinnerValue = parseInt(spinner.find(".filling-circle-number").text()),
                        rotation = tempSpinnerValue*3.6;

                    spinner.addClass("finished-loading");

                    if(tempSpinnerValue < 50){
                        console.log('1', tempSpinnerValue);
                        leftPie.css({'transform' : 'rotate('+ rotation +'deg)'});
                        setTimeout(function(){
                            leftPie.addClass("quick-finish-animation");
                            leftPie.css({'transform' : 'rotate('+ 180 +'deg)'});
                        },10);
                        setTimeout(function(){
                            rightPie.addClass("quick-finish-animation");
                            rightPie.css({'transform' : 'rotate('+ 180 +'deg)'});
                        },500);
                    }
                    else {
                        console.log('2', tempSpinnerValue);
                        setTimeout(function(){
                            leftPie.css({'transform' : 'rotate('+ 180 +'deg)'});
                            rightPie.css({'transform' : 'rotate('+ rotation +'deg)'});
                            rightPie.addClass("quick-finish-animation");
                            rightPie.css({'transform' : 'rotate('+ 180 +'deg)'});
                        },10);
                    }

                    $(".filling-circle-number-loaded").countTo({
                        from: tempSpinnerValue,
                        to: 100,
                        speed: 1000,
                        refreshInterval: 50,
                        onComplete: function() {
                            setTimeout(function(){
                                $('body > .mkd-smooth-transition-loader.mkd-mimic-ajax').fadeOut(500, 'easeInOutQuint');
                            },300);
                        }
                    });
                }, 200);
...
(function ($) {
	...
	shortcodes.mkdComparisonSlider = mkdComparisonSlider;
	...
	function mkdGetPortfolioAjaxData(container) {
		...
		returnValue.textLength = '';
		returnValue.textAlign = '';
		returnValue.filter = '';
		...
		if (typeof container.data('text-align') !== 'undefined' && container.data('text-align') !== false) {
			returnValue.textAlign = container.data('text-align');
		}
	...
	function mkdInitTextMarquee() {
		var marqueeSections = $('.mkd-text-marquee');
		...
		if (marqueeSections.length) {
			marqueeSections.each(function(){
				...
				var marqueeEffect = function () {
					...
					var marqueeText = marqueeSection.find('.mkd-text-marquee-title'),
						originalText = marqueeText.first(),
						auxText = marqueeText.filter('.mkd-aux-text'),
					...
					marqueeText.each(function(i){
						...
						var mkdInfiniteScrollEffect = function() {
							...
							requestAnimFrame(mkdInfiniteScrollEffect);
	...
	function mkdResizeProductMasonry(container) {
		...
		var defaultMasonryItem = container.find('.topfit_mikado_square');
		var largeWidthMasonryItem = container.find('.topfit_mikado_large_width');
		var largeHeightMasonryItem = container.find('.topfit_mikado_large_height');
		var largeWidthHeightMasonryItem = container.find('.topfit_mikado_large_width_height');
	...
	function mkdComparisonSlider() {
		var cmpSliders = $('.comparison-slider');
