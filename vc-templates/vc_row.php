...
if(!empty($row_type) && $row_type == 'parallax') {
	...
	if(topfit_mikado_options()->getOptionValue('parallax_on_off') == 'off') {
...
if($row_type == 'row' && $css_animation != '') {
	...
	if($transition_delay !== '') {
		...
		$inner_start .= '<div '.topfit_mikado_get_inline_style($animation_styles).'>';
		$inner_end .= '</div>';
	} else {
		$inner_start .= '<div>';
		$inner_end .= '</div>';
...
if($video == 'show_video') {
	...
	if($video_image) {
		$video_output .= '<div class="mkd-mobile-video-image" '.topfit_mikado_get_inline_attr($video_mobile_style, 'style').')"></div>';
	}
	$video_output .= '<div '.topfit_mikado_get_class_attribute($video_overlay_class).topfit_mikado_get_inline_attr($video_overlay_style, 'style').'></div>';
	$video_output .= '<div class="mkd-video-wrap">';
	$video_output .= '<video class="mkd-video" width="1920" height="800" '.topfit_mikado_get_inline_attr($video_attrs, 'poster').' controls="controls" preload="auto" loop autoplay muted>';
	if(!empty($video_webm)) {
	...
	if($v_image) {
		$video_output .= '<img '.topfit_mikado_get_inline_attr($v_image, 'src').' width="1920" height="800" title="'.esc_attr_e('No video playback capabilities', 'topfit').'" alt="'.esc_attr_e('Video thumb', 'topfit').'" />';
	}
	$video_output .= '</object>';
	$video_output .= '</video>';
	$video_output .= '</div>';
...
echo topfit_mikado_get_module_part($output);