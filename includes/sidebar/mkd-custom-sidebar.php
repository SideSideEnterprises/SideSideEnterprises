...
if(!class_exists('TopFitMikadoSidebar')) {

	class TopFitMikadoSidebar {
		...
		function __construct() {
			...
			$this->title  = esc_html__('Custom Widget Area', 'topfit');
		...
		function load_assets() {
			...
			wp_enqueue_script('mkd-sidebar', MIKADO_ROOT.'/framework/admin/assets/js/mkd-sidebar.js');
			wp_enqueue_style('mkd-sidebar', MIKADO_ROOT.'/framework/admin/assets/css/mkd-sidebar.css');
		}
		...
		function template_add_widget_field() {
			...
			echo "\n    <span class='input_wrap'><input type='text' value='' placeholder = '".esc_attr__('Enter Name of the new Widget Area', 'topfit')."' name='mkd-add-widget' /></span>";
			echo "\n    <input class='button' type='submit' value='".esc_attr__('Add Widget Area', 'topfit')."' />";
			echo "\n    ".$nonce;
			echo "\n  </form>";
			echo "\n</script>\n";
		}
		...
		function register_custom_sidebars() {
			...
			$args = apply_filters('topfit_mikado_custom_widget_args', $args);
